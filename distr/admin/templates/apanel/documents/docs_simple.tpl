...
<script language="javascript" type="text/javascript">
  function insertLink(target,value,doc,doctarget) {ldelim}
	...
	if (doc!='') window.opener.document.getElementById(doctarget).value = doc;
    window.close();
{rdelim}
...
<table width="100%" border="0" cellpadding="6" cellspacing="1" class="tableborder">
  ...
  <tr style="background-color: #eff3eb;" onmouseover="this.style.backgroundColor='#dae0d8';" onmouseout="this.style.backgroundColor='#eff3eb';" id="table_rows">
    ...
    <td nowrap="nowrap" width="1%" align="right">
      ...
      {elseif $smarty.request.idonly==2}
      <input onclick="insertLink('{$smarty.request.target}','{$item->Titel}','','{$smarty.request.doc}');" class="button" type="button" value="{#DOC_BUTTON_INSERT_LINK#}" />
      {else}
