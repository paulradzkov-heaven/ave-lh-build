...
{rdelim}
  ...
  function AddTags(object, field){ldelim}
  		var str = document.getElementById(field).value;
		if (str !== "") {ldelim}
			document.getElementById(field).value = str + ',' + object.text;
		{rdelim} else {ldelim}
			document.getElementById(field).value = str + object.text;
		{rdelim}
  {rdelim}
...
<form method="post" name="formDocOption" action="/admin/{$formaction}" enctype="multipart/form-data">
  <table width="100%" border="0" cellpadding="8" cellspacing="1" class="tableborder">
	...
	<tr>
      <td width="225" class="first">{#DOC_PARENT#}</td>
      <td class="second" nowrap><input name="Parent" type="text" id="Parent" size="40" style="width:275px" value="{if $row_doc->ParentTitle}{$row_doc->ParentTitle}{/if}" />&nbsp;
      <input onclick=" window.open('index.php?doc=Titel&target=Parent&do=docs&action=showsimple&cp={$sess}&pop=1&idonly=2','pop','left=0,top=0,width='+screen.width * 0.6+',height='+screen.height * 0.6+',scrollbars=1,resizable=1');" type="button" class="button" value="Добавить документ" />&nbsp;
        <input {popup width=350 sticky=false text=$config_vars.DOC_PARENT_INFO|default:''} class="button" style="cursor:help" type="button" value="?" /><br />
      </td>
      </td>
    </tr>
    <tr>
