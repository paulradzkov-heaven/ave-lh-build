<select name="ElterNavi" id="ElterNavi">
  <option value="0">{$name_empty}</option>
    ...
    <option value="{$ne->Id}" {if $row_doc->ElterNavi==$ne->Id}selected{/if}>&nbsp; {$ne->Titel|escape:html|stripslashes}</option>
  
    {foreach from=$ne->ebene_2 item=item_2}
    
    <option value="{$item_2->Id}" {if $row_doc->ElterNavi==$item_2->Id}selected{/if}>&nbsp;&nbsp;&nbsp;&nbsp;- {$item_2->Titel|escape:html|stripslashes} </option>
